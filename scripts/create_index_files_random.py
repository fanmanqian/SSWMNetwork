...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_audio', default='/home/ubuntu/codes/tmp/host_video_3/audio',
                        help="root for extracted audio files")
    parser.add_argument('--root_frame', default='/home/ubuntu/codes/tmp/host_video_3/frames224',
    ...
    male = ["guguoning", "heyanke", "mayaguang", "mingercha", "sangchaohui", "huangfeng", "zhuguangquan", "pantao",
            "zhengxuan"]
    female = ["baoxiaofeng", "haixia", "hejia", "hudie", "huwancong", "jingyi", "jinyi", "mulinshan",
              "nv", "ouyangxiadan", "shiyu", "weixian", "xuke", "zhengtianliang", "lizimeng", "kali"]
    ...
    audio_files += infos_male
    audio_files += infos_female

    infos = []

    for index, value in enumerate(range(4410)):
        # random.seed(index)
        ...
        if int(infos_male[index_male1].split(',')[-1]) > args.fps * 4 and int(
                infos_male[index_male2].split(',')[-1]) > args.fps * 4:
            tmp = audio_files[index_male1] + '|' + audio_files[index_male2]
            infos.append(tmp)
    ...
    for index, value in enumerate(range(4410)):
        ...
        if int(infos_female[index_female1].split(',')[-1]) > args.fps * 4 and int(
                infos_female[index_female2].split(',')[-1]) > args.fps * 4:
    ...
    for index, value in enumerate(range(4410)):
        ...
        if int(infos_male[index_male_female1].split(',')[-1]) > args.fps * 4 and int(
                infos_female[index_male_female2].split(',')[-1]) > args.fps * 4:
    ...
    for name, subset in zip(['final_train_random_face', 'final_test_random_face'], [trainset, valset]):
