...
class BaseDataset(torchdata.Dataset):
    ...
    def _stft(self, audio):
        ...
        spec = librosa.stft(audio, n_fft=self.stft_frame, hop_length=self.stft_hop, win_length=self.stft_win)
        amp = np.abs(spec)
        phase = np.angle(spec)
    ...
    def _load_audio_file(self, path):
        if path.endswith('.mp3'):
            ...
            # # audio_raw = audio_raw.reshape(audio_raw.shape[1], audio_raw.shape[0])
            #
            # # range to [-1, 1]
            # audio_raw *= (2.0**-31)
            # # convert to mono
    ...
    def dummy_mix_data(self, N):
        ...
        amp_mix = torch.zeros(1, 512, 320)
        phase_mix = torch.zeros(1, 512, 320)
