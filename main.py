...
def calc_metrics(batch_data, outputs, args):
    ...
    for j in range(B):
        ...
        if valid:
            pesq_stoi1.append(np.asarray(gts_wav))
            pesq_stoi2.append(np.asarray(preds_wav))
            ...
            #     pesq.append(pypesq(args.audRate, gt_wav, pred_wav, 'nb'))
            #     stoi1.append(stoi(gt_wav, pred_wav, args.audRate, extended=False))
            #
            #
            ...
            # pesq_meter.update(np.array(pesq).mean())
            # stoi_meter.update(np.array(stoi1).mean())
...
def evaluate(netWrapper, loader, history, epoch, args):
    ...
    for i, batch_data in enumerate(loader):
        ...
        # calculate metrics
        # sdr_mix, sdr, sir, sar, pesq, stoi = calc_metrics(batch_data, outputs, args)
        pesq, stoi = calc_metrics(batch_data, outputs, args)
        total_pesq_stoi1 += pesq
        total_pesq_stoi2 += stoi
    ...
    np.save(args.store_path + '/gt' + str(epoch) + '.npy', total_pesq_stoi1)
    np.save(args.store_path + '/pred' + str(epoch) + '.npy', total_pesq_stoi2)
    with open('metric_results.txt', 'a') as f:
        f.write(str(epoch) + '\t' + str(loss_meter.average()) + '\n')
...
def main(args):
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        # filename = 'train.prof'
        # cProfile.run('train(netWrapper, loader_train, optimizer, history, epoch, args)', filename)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        args.weights_sound = os.path.join('ckpt', 'sound_latest.pth')
        args.weights_frame = os.path.join('ckpt', 'frame_latest.pth')
        args.weights_synthesizer = os.path.join('ckpt', 'synthesizer_latest.pth')
